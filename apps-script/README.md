...
- 三處 `LockService` 的 `finally` 補上 `SpreadsheetApp.flush()`，避免寫入還沒落地就放鎖、
  被下一個請求讀到舊值
- `Sheets.gs` 的 `appendRow` / `updateRowById` 擋掉同一類的坑：寫值之前先把文字欄位
  鎖成純文字（防數字與日期轉型），`=` 開頭的值另外用單引號標記寫入（防被當成公式）

Sheets 的自動轉型其實是**兩件不同的事**，實測（`Repair.gs` 的 `probeCellWriteBehaviour()`）結果：

| | `@` 格式 + `setValue` | 前綴單引號 |
|---|---|---|
| `0012345`、`2026:10`、`2026-09`、`3/4` | 保得住 | 保得住 |
| `=1+1 測試`、`=SUM(A1:A9)` | **變 `#ERROR!`** | 保得住 |
| `+886912345678`、`-5V 沒輸出` | 保得住 | 保得住 |

`setValue` 對 `=` 開頭的字串一定當公式處理，**跟儲存格格式無關**（官方文件明寫）。
所以兩招都要用：`@` 格式擋數字與日期的轉型，`=` 開頭的值改用 Sheets 的「這是文字」
標記（值前面加一個單引號）寫入，讀回來不會帶著那個單引號。

`setRichTextValue` 試過但不能用——它會把 `+886912345678` 變成 `=+886912345678`，
反而生出一個公式。

兩個實作上的細節：

- `updateRowById` 掃的是**整列**，不是只掃這次 patch 到的欄位。`setValues` 是整列寫回去的，
  沒被改到的格子如果本來就存著 `=` 開頭的文字，這一寫就會被重新解讀成公式
- `=` 開頭的值刻意不「先寫進去再蓋掉」：使用者輸入的公式就算只存在一瞬間也可能被求值，
...
## 13. 作廢（cases.void / cases.unvoid）

開錯單、開重複很常見，但系統原本沒有任何移除案件的路徑（`deleteRowById` 一直沒人呼叫）。
補的是**作廢**，不是真刪：真刪一列會讓「這個料號到底出過什麼事」直接斷掉，
而那正是這個看板要回答的問題。

作廢用獨立欄位 `isVoided` / `voidedAt` / `voidedBy` / `voidReason`，**不是新增一個狀態**。
作廢跟「待處理→處理中→暫緩→已結案」是兩件事，混進 `status` 會讓統計卡、篩選、結案規則
都得跟著長出例外。做法與留言、附件的軟刪除一致。
...
開單人限「待處理」的理由：廠務一旦接手，就不該由開單人單方面撤掉。
已結案的一律不能作廢，要作廢請先把狀態改回去——結案紀錄不該被繞過。
復原只給廠務／admin：作廢是刻意的動作，要撤回得由能為它負責的人來。

### 行為

- 作廢原因**必填**。作廢的單之後還看得到，沒寫原因的話沒人知道當初為什麼撤掉
- 作廢的單是**凍結**的：`cases.update`、`cases.setStatus`、`comments.create` 都會擋下來，
  要繼續處理得先復原
- 既有留言與歷程照常顯示，作廢與復原本身也各留一筆 History
- `cases.list` 預設不回作廢的單；帶 `voided: true` 則**只**回作廢的那些
  （混在一起看不出差別，反而容易誤判）。前端在篩選列有一顆「已作廢」切換鈕
- `cases.stats` 不計入作廢的單：一張作廢的「待處理」不該讓待處理的數字看起來還有事要做
- 復原會把 `voidReason` 一併清掉，留著會跟「目前沒被作廢」互相矛盾，歷程裡查得到就夠了

**部署後要跑一次 `setupSheets()`**，Cases 分頁才會長出那四個新欄位。既有列的新欄位是空白的，
`isVoided` 空白等於沒作廢，不用回填。

## 14. 開站要等多久，以及為什麼

實測 `/exec` 的 `ping`（`doPost` 只回一個時間戳，什麼事都不做）：

| | 時間 |
|---|---|
| 冷啟動（容器被回收後第一次） | **8.34 秒** |
| 熱的時候 | 1.03 / 1.11 秒 |

量法（不用登入）：

```
curl -sL -o /dev/null -X POST -H 'Content-Type: text/plain' \
  -d '{"action":"ping"}' -w '%{time_total}\n' "<GAS_URL>"
```

拆解：

- **約 7 秒是 Apps Script 把容器叫醒**，跟我們的程式碼、跟資料量都無關。
  容器閒置一陣子就會被回收，所以早上第一次開、午休後回來開都會吃到。
  → `keepWarm()` + 每 5 分鐘的觸發器（選單「設定保溫排程」或執行 `ensureKeepWarmTrigger()`）
- **約 1 秒是 `/exec` 回應前那個 302 轉址**，這是 Apps Script Web App 的固定成本，省不掉。
  能做的只有「少跑幾趟」——`buildBoot_` 已經把開站的四趟併成一趟，`cases.update`
  也把改欄位、加附件、移除附件併成一個請求，都是為了這個
- **後端本身的運算不到 100ms**，六張表全部讀進來也才十幾列。優化後端邏輯沒有意義

前端這邊，開站等待時原本畫面停在登入閘門，看起來像在要使用者重新登入。改成
