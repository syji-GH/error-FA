...
- [ ] 編輯一張單（改料號 + 換掉一張圖）→ 主畫面只剩新圖，
      右側「處理紀錄」同一組裡看得到料號新舊值、舊圖縮圖，而且舊圖點得開
- [ ] **用廠務部帳號編輯別人的單 → 只看得到「承辦人 / 處理結果」，
      在 Console 直接呼叫 `cases.update` 送 `description` 必須被擋**（個人改個人的單）
- [ ] **用非 admin、非廠務部、非開單人的帳號在 Console 呼叫 `cases.update` → 必須被擋**
...
- **移除附件不會釋放 Drive 空間**。附件一律軟刪除，Drive 檔案刻意不丟垃圾桶，
  這樣「處理紀錄」裡的舊圖才點得開。真的要清掉得到 Drive 手動刪，
  刪掉之後那筆歷程的縮圖會變成「無法顯示」，其餘資訊（誰、何時、檔名）還在。
